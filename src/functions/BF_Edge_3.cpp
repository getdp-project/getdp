...
int Get_FacetFunctionIndexPeriodic(struct Element *Element, int NumEntity, int NumIndex)
{
  ...
  if(it == _periodicFacets.end()){
    ...
    // the default scheme (?) 
    Message::Info("key from %s",key.c_str());
    int local_ids[3];
    for(int i = 0; i < 3; ++i)
      local_ids[i] = loc[i];
    ...
    Message::Info("periodic facet %d tri {%d,%d,%d} bary={%.2f,%.2f,%.2f}",NumEntity, g[0],g[1],g[2],bx,by,bz);
...
// used in branch linkfacets
int Get_FacetFunctionIndexPeriodicGeomSort(struct Element *Element, int NumEntity, int NumIndex)
{
#if !defined(HAVE_KERNEL)
  Message::Error("Get_FacetFunctionIndex requires Kernel");
  return 0;
